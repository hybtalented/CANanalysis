...
class CANREAD{
	...
	BYTE ifs,ofs;
public:
	CANREAD():canframes(),oldchm(),oldbhm(),oldcrm(),oldcts(),oldcml(),oldbro(),oldbcl(),oldccs(),oldbsm(),oldbst(),oldcst(),oldbsd(),oldcsd(),oldbem(),oldcem(),oldbrm(),oldbcp(),oldbcs(),multiframe(NULL),oldPGN(0xff),autoindex(false),indexcol(0),ifs('\t'),ofs('\t'){}
	...
	VOID print(ostream&);
	~CANREAD() { if (multiframe) { delete multiframe; multiframe = NULL; } for (vector<Basic_CAN_frame*>::iterator it = canframes.begin(); it != canframes.end(); it++)delete *it; canframes.clear(); }
};
