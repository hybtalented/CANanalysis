...
CONST_CHARPTR Basic_CAN_frame::states[8]={"充电握手阶段","充电辨识阶段","充电配置阶段","充电阶段","充电结束阶段","错误报文","多帧传输","未定义"};
...
string CAN_CTS::getDetail(int i) const {
	...
	switch (i) {
	case 0:
		oss << setw(4) << GET_BCD_WORD(year) << "年" << setw(2) << (UINT)GET_BCD_BYTE(month) << "月" << setw(2) << (UINT)GET_BCD_BYTE(day) << "日" << setw(2) << (UINT)GET_BCD_BYTE(hour) << "时" << setw(2) << (UINT)GET_BCD_BYTE(minute) << "分" << setw(2) << (UINT)GET_BCD_BYTE(second) << "秒";
		break;
	}
	return oss.str().c_str();
}
...
string CAN_BSM::get_detail()const{
	...
	if(maxtem != 0xff)
		oss<<"最高电池温度编号:0x"<< hex<<setw(2)<< (UINT)maxtemid << ",其温度为:"<<(UINT)maxtem - 50<<"摄氏度.";
	if(mintem != 0xff)
			oss<<"最低电池温度编号:0x"<< hex<<setw(2)<< (UINT)mintemid << ",其温度为:"<<(UINT)mintem - 50<<"摄氏度.";
	oss<<"当前动力蓄电池状态--";
...
string CAN_CSD::get_detail()const{
	...
	if(CM_id != 0xffffffff)
		oss<<"充电机编号:0x"<<hex<<setw(8)<<setfill('0')<<CM_id<<'.';
	return oss.str();
}
...
string CAN_TP_START::get_detail()const{
	...
	if(console == TP_START_CONSOLE)
		oss<<"多帧传输总字节数："<<length<<",数据包数:"<<(UINT)packets<<".传输包的PGN为0x00"<<hex<<setw(2)<<setfill('0')<<int(PGN)<<"00.";
	else
		oss<<"解析错误，并非多包数据请求帧！！！";
	return oss.str();
}
...
string CAN_TP_REPLY::get_detail()const{
	...
	if(compts.reply.console == TP_REPLY_CONSOLE)
		oss<<"多帧传输剩余数据包数："<<(UINT)compts.reply.packets<<".多帧传输下一数据包编号为:"<<(UINT)compts.reply.nextpacket<<".传输包的PGN为0x00"<<hex<<setw(2)<< setfill('0') <<int(compts.reply.PGN)<<"00.";
	else if(compts.end.console == TP_END_CONSOLE)
		oss<<"已传输总字节数："<<(UINT)compts.end.length<<",数据包数:"<<(UINT)compts.end.packets<<".传输包的PGN为0x00"<<hex<<setw(2)<<setfill('0')<<int(compts.end.PGN)<<"00.";
	else
		oss<<"解析错误，并非多包数据回复帧！！！";
	return oss.str();
}
