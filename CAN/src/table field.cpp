...
VOID Table_field::initFields(){
	...
	for(UINT i=0,size=field.size() ; i < size; ++i ){
			while( i <size && (field[i] == ifs|| field[i] == ' ' || field[i] == '\t') && field[ i ] !='\0'  ) ++i ;
			fields.push_back(i);
			while(i <size && (field[i] != ifs && field[i] != ' ' && field[i] != '\t') && field[ i ] !='\0')  ++i ;
			if( i< size)
				field[ i ] ='\0';
...
Table_field::Table_field(istream& in , BYTE fs , BOOL stand ):ifs(fs),ofs('\t'){
	getline(in, field);
	if(stand){
			initFieldsStandard();
	}else{
...
Table_field::Table_field(string&& s,BYTE fs, BOOL stand ):field(s),ifs(fs){
	if(stand){
		initFieldsStandard();
	}else{
		initFields();
